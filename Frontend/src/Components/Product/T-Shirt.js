...
//   // Fetch products from API (or use static data if needed)
//   useEffect(() => {
//     const fetchProducts = async () => {
//       try {
//         const response = await axios.get('http://localhost:5000/api/products/T-Shirt');
...
//                   <li className="active"><Link to="/t-shirt">T-Shirt</Link></li>
...
function TShirt() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // const response = await axios.get('http://localhost:5000/api/products/T-Shirt');
        const response = await axios.get(`${process.env.REACT_APP_PRODUCT_API_BASE_URL}/api/products/T-Shirt`);
